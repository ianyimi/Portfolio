...
CLOUD_URL="${S3}/models/${CLOUD_FOLDER}/${FILE_NAME}.glb.gz"
...
# upload to s3
aws s3 cp "${STAGED_FILE}" "s3://ianyimi-portfolio/models/${CLOUD_FOLDER}/${FILE_NAME}.glb"|| handle_error
aws s3 cp "${STAGED_FILE}.gz" "s3://ianyimi-portfolio/models/${CLOUD_FOLDER}/${FILE_NAME}.glb.gz" --content-encoding "gzip" || handle_error
