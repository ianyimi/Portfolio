...
function Rig() {
  ...
  const [targetPosVec] = useState(() => new THREE.Vector3());
  const {scene, camera} = useThree();
  const dummy = useRef(null);
  const dummy2 = useRef(null);

  const targetRaycaster = new THREE.Raycaster();
  targetRaycaster.setFromCamera(new Vector2(), camera);
  targetRaycaster.ray.at(5, targetPosVec);
  if (dummy2.current) dummy2.current.position.set(targetPosVec.x, targetPosVec.y, targetPosVec.z);
  ...
  useEffect(() => {
    ...
    function handleMouseMove(event: MouseEvent) {
      ...
      raycaster.ray.at(0.5, quatPosVec);


      dummy.current.position.set(quatPosVec.x, quatPosVec.y, quatPosVec.z)
      dummy.current.lookAt(targetPosVec);
      // dummy.current.getWorldQuaternion(newQuaternion);
      // newQuaternion.copy(dummy.current.quaternion);
      console.log(newQuaternion);
      // camera.quaternion.slerp(targetPosVec, 0.05);
  ...
  return <group>
    <mesh ref={dummy} visible={true}>
      <boxBufferGeometry args={[0.1, 0.1, 0.1]}/>
      <meshBasicMaterial color="blue"/>
    </mesh>
    <mesh ref={dummy2} visible={true}>
